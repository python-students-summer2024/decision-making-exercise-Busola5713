...
def is_square():
    ...
    width = float(width) or int(width)
    height = float(height) or int(height)
    # check if user input is integer or floating number
    if width == int(width) and height == int(height):
        if width == height:
            return True
        else:
            return False
    elif width == float(width) and height == float(height):
         if width == height:
            return True
         else:
            return False
...
def get_greatest():
    ...
    """
    #### write your solution for this function below here. ####
    greatest_1 = int(input("Enter integer #1: "))
    greatest_2 = int(input("Enter integer #2: "))
    if greatest_1 > greatest_2:
        return greatest_1
    elif greatest_2 > greatest_1:
        return greatest_2
    else:
        return greatest_1
...
def get_bmi_category():
    ...
    """
    #### write your solution for this function below here. ####
    heights = input("Enter your height (in inches): ")
    weight = input("Enter your weight (in pounds): ")
    heights = int(heights) or float(heights)
    weight = int(weight) or float(weight)
    # check if user input is integer or floating number
    if (weight == int(weight) or weight == float(weight)) and (heights == int(heights) or heights == float(heights)):
        bmi = 703 * weight / heights ** 2
        if bmi < 15:
...
def get_discount():
    ...
    mask_price = masks * 5
    if 5000 <= masks:
         discount = mask_price - (mask_price * 0.20)
         discount = int(round(discount))
         return '${:,}'.format(discount)
    else:
        mask_price = int(round(mask_price))
        return '${:,}'.format(mask_price)
